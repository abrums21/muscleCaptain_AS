...
public class PuzzleGame {
    ...
    private List<PuzzleBlock> blockList = new ArrayList<PuzzleBlock>();
    ...
    public PuzzleGame(Context context, int colums, Bitmap oldBitmap) {
    ...
    private void getWidthHeight(Context context, float[] data) {
        DisplayMetrics metric = ScreenUtil.getScreenMetrics(context);
    ...
    private void getPuzzleBitmap() {
        ...
        for (int i = 1; i <= colums; i++) {
            for (int j = 1; j <= colums; j++) {
                ...
                blockList.add(new PuzzleBlock((i - 1) * colums + j, (i - 1) * colums + j, tmp));
            }
        }
        ...
        tmp = BitmapFactory.decodeResource(context.getResources(), R.drawable.tool_puzzle_blank, options);
        tmp = Bitmap.createBitmap(tmp, 0, 0, itemWidth, itemHeight);

        blockList.add(new PuzzleBlock(colums * colums, 0, tmp));
    ...
    private static int getCheckSum(List<PuzzleBlock> data) {
        int sum = 0;
        int iDataId;
        int jDataId;
