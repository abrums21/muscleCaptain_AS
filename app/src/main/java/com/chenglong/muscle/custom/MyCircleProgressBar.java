...
public class MyCircleProgressBar extends View {
    ...
    public MyCircleProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ar = context.obtainStyledAttributes(attrs, R.styleable.MyCircleProgressBar, defStyleAttr, 0);
        String mTextRead = ar.getString(R.styleable.MyCircleProgressBar_text);
        if (mTextRead != null)
        {
            mText = mTextRead;
        }

        mStrokeWidth = ar.getDimension(R.styleable.MyCircleProgressBar_strokeWidth, MyScreenUtil.sp2px(context, STROKE_WIDTH));
        ar.recycle();

//        mStrokeWidth = MyScreenUtil.sp2px(context, mStrokeWidthRead);
