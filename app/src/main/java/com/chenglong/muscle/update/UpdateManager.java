...
public class UpdateManager {
    ...
    public UpdateManager(Context context) {
        ...
        mApkHandle = new ApkHandle(mContext);
        try {
    ...
    private void showUpdateDialog() {
        ...
        AlertDialog dialog = builder.setTitle("应用版本有更新").setIcon(R.drawable.menu_main_1).setMessage(map.get("info").toString())
...
//		notification.contentView = mRemoteViews;
    ...
    private void getDownloadInfo() {
        PatchCfg mPatchCfg = (PatchCfg) (map.get(MyPackageUtil.getAppVersionCode(mContext) + ""));
        if ((null == mPatchCfg) || (PATH.isEmpty()) || (!mApkHandle.apkIsExist(mPatchCfg.getMd5()))) {
            downloadUrl = map.get("url").toString();
            downloadName = PATH + map.get("name") + ".apk";
        } else {
            patch = true;
            downloadUrl = mPatchCfg.getUrl();
            downloadName = PATH + mPatchCfg.getName() + ".apk";
        }
    ...
    private boolean patchApk() {
		...
		/* 拼装apk */
        boolean ret = mApkHandle.mergeApk(PATH + "old.apk", installName, downloadName);
